...
import type { IOption, ISelectProps } from "@/models/selects";
...
export const Select: React.FC<ISelectProps> = ({
  ...
  return (
    ...
    >
      <div
        className="flex min-h-11.5 cursor-pointer items-center justify-between gap-0.5 rounded-lg border border-border bg-background px-3 py-2.5 hover:border-primary"
          ...
          {!isEmpty ? (
            isMulti ? (
              selectedArray.map((v) => (
                <span
                  key={v.value}
                  className="flex items-center gap-1 rounded bg-success py-0.5 pr-1 pl-2 text-sm text-text-highlight"
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            className={`mt-1 w-full overflow-hidden rounded-lg border border-border bg-foreground shadow-lg ${
